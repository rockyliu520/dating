...
;(function ($, Colors, undefined) {
	...
	var $document = $(document),
	    ...
	    _html = '<div class="cp-color-picker"><div class="cp-z-slider"><div c' + 'lass="cp-z-cursor"></div></div><div class="cp-xy-slider"><div cl' + 'ass="cp-white"></div><div class="cp-xy-cursor"></div></div><div ' + 'class="cp-alpha"><div class="cp-alpha-cursor"></div></div></div>',
	...
	function extractValue(elm) {
		return elm.value || elm.getAttribute('value') || $(elm).css('background-color') || '#fff';
	}
	...
	function toggle(event) {
		...
		if (event) {
			...
			(_$UI || build()).css({
				// 'width': _$UI[0]._width,
				'left': (_$UI[0]._left = position.left) - ((_$UI[0]._left = _$UI[0]._left + _$UI[0]._width - ($window.scrollLeft() + $window.width())) + gap > 0 ? _$UI[0]._left + gap : 0),
				'top': (_$UI[0]._top = position.top + $this.outerHeight()) - ((_$UI[0]._top = _$UI[0]._top + _$UI[0]._height - ($window.scrollTop() + $window.height())) + gap > 0 ? _$UI[0]._top + gap : 0)
	...
	function build() {
		$('head').append('<style type="text/css">' + (_options.css || _css) + (_options.cssAddon || '') + '</style>');

		return _colorPicker.$UI = _$UI = $(_html).css({ 'margin': _options.margin }).appendTo('body').show(0, function () {
			var $this = $(this);
			...
			$this.prepend('<div>').children().eq(0).css('width', $this.children().eq(0).width() // stabilizer
			);
			this._width = this.offsetWidth;
			this._height = this.offsetHeight;
		}).hide().on(_pointerdown, '.cp-xy-slider,.cp-z-slider,.cp-alpha', pointerdown);
	}

	function pointerdown(e) {
		var action = this.className.replace(/cp-(.*?)(?:\s*|$)/, '$1').replace('-', '_');

		e.preventDefault && e.preventDefault();
		e.returnValue = false;

		_$trigger._offset = $(this).offset();
		(action = action === 'xy_slider' ? xy_slider : action === 'z_slider' ? z_slider : alpha)(e);
	...
	function preRender(toggled) {
		var colors = _color.colors,
		    ...
		    text = colorMode === 'HEX' && !isAlpha ? '#' + colors.HEX : colorMode === 'rgb' || colorMode === 'HEX' && isAlpha ? !isAlpha ? 'rgb(' + RGBInnerText + ')' : 'rgba(' + RGBInnerText + ', ' + alpha + ')' : 'hsl' + (isAlpha ? 'a(' : '(') + HSL.h + ', ' + HSL.s + '%, ' + HSL.l + '%' + (isAlpha ? ', ' + alpha : '') + ')',
		    ...
		    hasNoValue = _$trigger[0].hasAttribute('value') && triggerValue === '' && toggled !== undefined;
		...
		_$trigger._css = {
			...
			color: hasNoValue ? '' : colors.rgbaMixBGMixCustom.luminance > 0.22 ? dark : light
		};
	...
	function render(toggled) {
		...
		_options.renderCallback.call(_colorPicker, _$trigger, typeof toggled === 'boolean' ? toggled : undefined);
	}
	...
	$.fn.colorPicker = function (options) {
		...
		!_colorPicker && options.scrollResize && $(window).on('resize.a scroll.a', function () {
			if (_colorPicker.$trigger) {
				_colorPicker.toggle.call(_colorPicker.$trigger[0], true);
			}
		});
		_instance = _instance ? _instance.add(this) : this;
		_instance.colorPicker = _colorPicker || (_colorPicker = new ColorPicker(options));
		_selector += (_selector ? ', ' : '') + this.selector;

		$(options.body).off('.a').on(_pointerdown, function (e) {
			var $target = $(e.target);

			if ($.inArray($target.closest(_selector)[0], _instance) === -1 && !$target.closest(_$UI).length) {
				toggle();
			}
		}).on('focus.a click.a', _selector, toggle).on('change.a', _selector, function () {
		...
		return this.each(function () {
			...
			$elm.data('colorMode', mode[1] ? mode[0].substr(0, 3) : 'HEX').attr('readonly', _options.preventFocus);
			options.doRender && $elm.css({ 'background-color': value,
				'color': function color() {
					return _color.setColor(value).rgbaMixBGMixCustom.luminance > 0.22 ? '#222' : '#ddd';
				}
			});
		});
	};
